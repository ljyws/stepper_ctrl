...
{
    uint32_t cnt_;		
    for(cnt_ = 0; cnt_ < MOVE_STEP_NUM + 1; cnt_++)
	{
		encode_cali.coder_data_f[cnt_] = (uint16_t)cycle_average((int32_t)encode_cali.coder_data_f[cnt_], (int32_t)encode_cali.coder_data_r[cnt_], CALI_ENCODER_RES);
	}

    int32_t sub_data = cycle_sub((int32_t)encode_cali.coder_data_f[0], (int32_t)encode_cali.coder_data_f[MOVE_STEP_NUM-1], CALI_ENCODER_RES);
	if(sub_data == 0)	
	{	
	...
	for(cnt_ = 1; cnt_ < MOVE_STEP_NUM; cnt_++)
	{
		sub_data = cycle_sub((int32_t)encode_cali.coder_data_f[cnt_], (int32_t)encode_cali.coder_data_f[cnt_-1], CALI_GATHER_ENCODER_RES);
		if(abs(sub_data) > (CALI_GATHER_ENCODER_RES * 3 / 2))	
		{	
			encode_cali.error_code = CALI_ERROR_AVERAGE_CONTINUITY;	
			encode_cali.error_data = cnt_;	
			return;
		}
	...
	if(encode_cali.dir)
	{
		for(cnt_ = 0; cnt_ < MOVE_STEP_NUM; cnt_++)
		{
			sub_data = (int32_t)encode_cali.coder_data_f[cycle_rem(cnt_+1, MOVE_STEP_NUM)] - (int32_t)encode_cali.coder_data_f[cycle_rem(cnt_, MOVE_STEP_NUM)];
			if(sub_data < 0)
			{
				step_num++;
				encode_cali.rcd_x = cnt_;
...
void encoder_calibration_interrupt_callback(void)
{
    ...
    case CALI_FORWARE_MEASURE:
    {
        if((encode_cali.out_location % MOVE_DIVEDE_NUM) == 0)
        {
            ...
            {
                encode_cali.coder_data_f[(encode_cali.out_location - MOVE_PULSE_NUM) / MOVE_DIVEDE_NUM] \
                    = cycle_data_average(encode_cali.coder_data_gather, ENC_CALI_GATHER_QUANTITY, CALI_ENCODER_RES);
    ...
    case CALI_REVERSE_MEASURE:
    {
		if((encode_cali.out_location % MOVE_DIVEDE_NUM) == 0)
		{
			...
			if(encode_cali.gather_count == ENC_CALI_GATHER_QUANTITY)
            {
				encode_cali.coder_data_r[(encode_cali.out_location - MOVE_PULSE_NUM) / MOVE_DIVEDE_NUM]
					= cycle_data_average(encode_cali.coder_data_gather, ENC_CALI_GATHER_QUANTITY, CALI_ENCODER_RES);
				encode_cali.gather_count = 0;
				encode_cali.out_location -= calibration_speed_;
			}
